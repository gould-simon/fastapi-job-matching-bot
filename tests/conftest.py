...
async def db_engine(temp_dir: Path):
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            await conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_job_matches_telegram_id ON job_matches(telegram_id)"
                )
            )
            await conn.execute(
                text(
...
async def db_session(temp_dir):
    ...
    engine = create_async_engine(f"sqlite+aiosqlite:///{test_db_path}", echo=True)
...
async def test_jobs(db_session, test_firm):
    ...
    for i in range(3):
        job = Job(
            ...
            date_published=datetime.now(timezone.utc) - timedelta(days=i),
            req_no=f"REQ-{i}",
        )
        jobs.append(job)
...
async def test_user_search(db_session, test_user):
    ...
    search = UserSearch(
        ...
        structured_preferences=json.dumps(
            {"location": "New York", "seniority": "Senior"}
        ),
    )
...
async def test_user_conversation(db_session, test_user):
    ...
    messages = [
        ...
        ("Here's my CV", True),
        (
            "Thanks, I've processed your CV. What kind of job are you looking for?",
            False,
        ),
    ]
